...
6. **Dynamic RBAC & Policy Auto-Discovery**: Phân quyền động dựa trên trait `HasPermissions` và `Gate::guessPolicyNamesUsing()`. Danh mục mã quyền (`permission_code`) được quản lý bất biến trong Code (`PermissionSeeder`), không tạo qua CRUD tự do trên UI. Xem chi tiết tại [Tài liệu Phân Quyền Động & RBAC Developer Guide](permissions-and-rbac.md).
